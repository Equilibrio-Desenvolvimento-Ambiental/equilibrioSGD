<?php
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$sql_CLASS415421_INTERF = mysql_query("SELECT * FROM TAB_415421_CLASSIFICACAO WHERE CLASS415421_CODIGO = '$id' AND EVENTOS_CODIGO = '$id_evento';", $db) or die(mysql_error());
			$vetor_CLASS415421_INTERF = mysql_fetch_array($sql_CLASS415421_INTERF);
?>
...
function habilitarSelect() {
	$("#tipo415421_interf").prop("disabled", false);
	$("#subtipo415421_interf").prop("disabled", false);
}

$(document).ready(function(){
	var cat = $('#tipo415421_interf').val();
	var subcat = $('#subtipo415421_interf').val();
	
	$('#tipo415421_interf').change(function(){
		$('#subtipo415421_interf').load('busca415421_interf_completo.php?id='+$('#tipo415421_interf').val());
	});
    ...
    $("#disable").click(function (){
		if ($('#subtipo415421_interf').val() == 0) {
			alert("SubAtividade é Inválida");
		} else {
			$("#tipo415421_interf").prop("disabled", true);
			$("#subtipo415421_interf").prop("disabled", true);
		}
	});
});
</script>
...
<?php require_once("includes/site-header.php");?>
	<div class="page-content">
		<div class="container-fluid">
					...
					<div class="tbl-row">
						<div class="tbl-cell">
							...
							<ol class="breadcrumb breadcrumb-simple">
								...
								<li><a href="#">Alteração de Dados de Atividade - Rural//Interface</a></li>
							</ol>
						</div>
					</div>
				</div>
			...
			<div class="box-typical box-typical-padding">
            	<form action="recebe_alterar_evento_class415421_interf.php?id=<?php echo $id; ?>&id_evento=<?php echo $id_evento; ?>&id_familia=<?php echo $id_familia; ?>" method="post" name="recebe_alterar_evento_class415421_interf" enctype="multipart/form-data" id="recebe_alterar_evento_class415421_interf">
                    <div style="width: 100%; margin: 0 auto;">
                        <ul class="tabs" data-persist="true">
                            <li><a href="#view1">Rural//Interface</a></li>
                        </ul>
                        ...
                        <table width="100%">
						  ...
						  <tr>
                            ...
                            <select name="tipo415421_interf" id="tipo415421_interf" class="form-control" disabled/>
                                <?php
									$subvisit415421_interf = $vetor_CLASS415421_INTERF['CLASS415421_TIPO'];
									$result01 = mysql_query("SELECT ID_PRINCIPAL FROM TAB_APOIO_TPSUBVISIT415421_INTERF WHERE ID = '$subvisit415421_interf';", $db) or die(mysql_error());
									$row01 = mysql_fetch_array($result01);
									$result02 = mysql_query("SELECT * FROM TAB_APOIO_TPVISIT415421_INTERF ORDER BY DESCRICAO ASC;", $db) or die(mysql_error());
							...
							<td width='28%' class='style12'>
                           	<select name="subtipo415421_interf" id="subtipo415421_interf" class="form-control" disabled/>
                                <?php
									$subresult = mysql_query("SELECT * FROM TAB_APOIO_TPSUBVISIT415421_INTERF ORDER BY DESCRICAO ASC;", $db) or die(mysql_error());
									while($subrow = mysql_fetch_array($subresult)){
										echo "<option ";
										if (strcasecmp($vetor_CLASS415421_INTERF['CLASS415421_TIPO'], $subrow['ID']) == 0){
											echo 'selected="selected"';
										} 
                            ...
                            <td width='30%'><textarea rows='4' class='form-control' name='CLASS415421_DESCRICAO'><?php echo $vetor_CLASS415421_INTERF['CLASS415421_DESCRICAO']; ?></textarea></td>
