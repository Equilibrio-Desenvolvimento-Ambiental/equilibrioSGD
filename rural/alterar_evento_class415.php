<?php
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$sql_CLASS415 = mysql_query("SELECT * FROM TAB_415_CLASSIFICACAO WHERE CLASS415_CODIGO = '$id' AND EVENTOS_CODIGO = '$id_evento';", $db) or die(mysql_error());
			$vetor_CLASS415 = mysql_fetch_array($sql_CLASS415);
?>
...
function habilitarSelect() {
	$("#tipo415").prop("disabled", false);
	$("#subtipo415").prop("disabled", false);
}

$(document).ready(function(){
	var cat = $('#tipo415').val();
	var subcat = $('#subtipo415').val();
	
	$('#tipo415').change(function(){
		$('#subtipo415').load('busca415_completo.php?id='+$('#tipo415').val());
	});
    ...
    $("#disable").click(function (){
		if ($('#subtipo415').val() == 0) {
			...
			$("#tipo415").prop("disabled", true);
			$("#subtipo415").prop("disabled", true);
		}
	});
});
</script>
...
<?php require_once("includes/site-header.php");?>
	<div class="page-content">
		<div class="container-fluid">
					...
					<div class="tbl-row">
						<div class="tbl-cell">
							...
							<ol class="breadcrumb breadcrumb-simple">
								...
								<li><a href="#">Alteração de Dados de Atividade - Reparação</a></li>
							</ol>
						</div>
					</div>
				</div>
			...
			<div class="box-typical box-typical-padding">
            	<form action="recebe_alterar_evento_class415.php?id=<?php echo $id; ?>&id_evento=<?php echo $id_evento; ?>&id_familia=<?php echo $id_familia; ?>" method="post" name="recebe_alterar_evento_class415" enctype="multipart/form-data" id="recebe_alterar_evento_class415">
                    <div style="width: 100%; margin: 0 auto;">
                        ...
                        <table width="100%">
						  ...
						  <tr>
                            ...
                            <select name="tipo415" id="tipo415" class="form-control" disabled/>
                                <?php
									$subvisit415 = $vetor_CLASS415['CLASS415_TIPO'];
									$result01 = mysql_query("SELECT ID_PRINCIPAL FROM TAB_APOIO_TPSUBVISIT415 WHERE ID = '$subvisit415';", $db) or die(mysql_error());
									$row01 = mysql_fetch_array($result01);
									$result02 = mysql_query("SELECT * FROM TAB_APOIO_TPVISIT415 ORDER BY DESCRICAO ASC;", $db) or die(mysql_error());
							...
							<td width='28%' class='style12'>
                           	<select name="subtipo415" id="subtipo415" class="form-control" disabled/>
                                <?php
									$subresult = mysql_query("SELECT * FROM TAB_APOIO_TPSUBVISIT415 ORDER BY DESCRICAO ASC;", $db) or die(mysql_error());
									while($subrow = mysql_fetch_array($subresult)){
										echo "<option ";
										if (strcasecmp($vetor_CLASS415['CLASS415_TIPO'], $subrow['ID']) == 0){
											echo 'selected="selected"';
										} 
                            ...
                            <td width='30%'><textarea rows='4' class='form-control' name='CLASS415_DESCRICAO'><?php echo $vetor_CLASS415['CLASS415_DESCRICAO']; ?></textarea></td>
