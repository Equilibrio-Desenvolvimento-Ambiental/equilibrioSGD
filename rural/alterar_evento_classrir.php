<?php
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$sql_CLASSRIR = mysql_query("SELECT * FROM TAB_RIR_CLASSIFICACAO WHERE CLASSRIR_CODIGO = '$id' AND EVENTOS_CODIGO = '$id_evento';", $db) or die(mysql_error());
			$vetor_CLASSRIR = mysql_fetch_array($sql_CLASSRIR);
?>
...
function habilitarSelect() {
	$("#tiporir").prop("disabled", false);
	$("#subtiporir").prop("disabled", false);
}

$(document).ready(function(){
	var cat = $('#tiporir').val();
	var subcat = $('#subtiporir').val();
	
	$('#tiporir').change(function(){
		$('#subtiporir').load('buscarir_completo.php?id='+$('#tiporir').val());
	});
    ...
    $("#disable").click(function (){
		if ($('#subtiporir').val() == 0) {
			...
			$("#tiporir").prop("disabled", true);
			$("#subtiporir").prop("disabled", true);
		}
	});
});
</script>
...
<?php require_once("includes/site-header.php");?>
	<div class="page-content">
		<div class="container-fluid">
					...
					<div class="tbl-row">
						<div class="tbl-cell">
							...
							<ol class="breadcrumb breadcrumb-simple">
								...
								<li><a href="#">Alteração de Dados de Atividade - RIR (1ª Versão)</a></li>
							</ol>
						</div>
					</div>
				</div>
			...
			<div class="box-typical box-typical-padding">
            	<form action="recebe_alterar_evento_classrir.php?id=<?php echo $id; ?>&id_evento=<?php echo $id_evento; ?>&id_familia=<?php echo $id_familia; ?>" method="post" name="recebe_alterar_evento_classrir" enctype="multipart/form-data" id="recebe_alterar_evento_classrir">
                    <div style="width: 100%; margin: 0 auto;">
                        <ul class="tabs" data-persist="true">
                            <li><a href="#view1">RIR (1ª Versão)</a></li>
                        </ul>
                        ...
                        <table width="100%">
						  ...
						  <tr>
                            ...
                            <select name="tiporir" id="tiporir" class="form-control" disabled/>
                                <?php
									$subvisitrir = $vetor_CLASSRIR['CLASSRIR_TIPO'];
									$result01 = mysql_query("SELECT ID_PRINCIPAL FROM TAB_APOIO_TPSUBVISITRIR WHERE ID = '$subvisitrir';", $db) or die(mysql_error());
									$row01 = mysql_fetch_array($result01);
									$result02 = mysql_query("SELECT * FROM TAB_APOIO_TPVISITRIR ORDER BY DESCRICAO ASC;", $db) or die(mysql_error());
							...
							<td width='28%' class='style12'>
                           	<select name="subtiporir" id="subtiporir" class="form-control" disabled/>
                                <?php
									$subresult = mysql_query("SELECT * FROM TAB_APOIO_TPSUBVISITRIR ORDER BY DESCRICAO ASC;", $db) or die(mysql_error());
									while($subrow = mysql_fetch_array($subresult)){
										echo "<option ";
										if (strcasecmp($vetor_CLASSRIR['CLASSRIR_TIPO'], $subrow['ID']) == 0){
											echo 'selected="selected"';
										} 
                            ...
                            <td width='30%'><textarea rows='4' class='form-control' name='CLASSRIR_DESCRICAO'><?php echo $vetor_CLASSRIR['CLASSRIR_DESCRICAO']; ?></textarea></td>
