<?php
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$sql_CLASS421 = mysql_query("SELECT * FROM TAB_421_CLASSIFICACAO WHERE CLASS421_CODIGO = '$id' AND EVENTOS_CODIGO = '$id_evento';", $db) or die(mysql_error());
			$vetor_CLASS421 = mysql_fetch_array($sql_CLASS421);
?>
...
function habilitarSelect() {
	$("#tipo421").prop("disabled", false);
	$("#subtipo421").prop("disabled", false);
}

$(document).ready(function(){
	var cat = $('#tipo421').val();
	var subcat = $('#subtipo421').val();
	
	$('#tipo421').change(function(){
		$('#subtipo421').load('busca421_completo.php?id='+$('#tipo421').val());
	});
    ...
    $("#disable").click(function (){
		if ($('#subtipo421').val() == 0) {
			...
			$("#tipo421").prop("disabled", true);
			$("#subtipo421").prop("disabled", true);
		}
	});
});
</script>
...
<?php require_once("includes/site-header.php");?>
	<div class="page-content">
		<div class="container-fluid">
					...
					<div class="tbl-row">
						<div class="tbl-cell">
							...
							<ol class="breadcrumb breadcrumb-simple">
								...
								<li><a href="#">Alteração de Dados de Atividade - ATES</a></li>
							</ol>
						</div>
					</div>
				</div>
			...
			<div class="box-typical box-typical-padding">
            	<form action="recebe_alterar_evento_class421.php?id=<?php echo $id; ?>&id_evento=<?php echo $id_evento; ?>&id_familia=<?php echo $id_familia; ?>" method="post" name="recebe_alterar_evento_class421" enctype="multipart/form-data" id="recebe_alterar_evento_class421">
                    <div style="width: 100%; margin: 0 auto;">
                        <ul class="tabs" data-persist="true">
                            <li><a href="#view1">ATES</a></li>
                        </ul>
                        ...
                        <table width="100%">
						  ...
						  <tr>
                            ...
                            <select name="tipo421" id="tipo421" class="form-control" disabled/>
                                <?php
									$subvisit421 = $vetor_CLASS421['CLASS421_TIPO'];
									$result01 = mysql_query("SELECT ID_PRINCIPAL FROM TAB_APOIO_TPSUBVISIT421 WHERE ID = '$subvisit421';", $db) or die(mysql_error());
									$row01 = mysql_fetch_array($result01);
									$result02 = mysql_query("SELECT * FROM TAB_APOIO_TPVISIT421 ORDER BY DESCRICAO ASC;", $db) or die(mysql_error());
							...
							<td width='28%' class='style12'>
                           	<select name="subtipo421" id="subtipo421" class="form-control" disabled/>
                                <?php
									$subresult = mysql_query("SELECT * FROM TAB_APOIO_TPSUBVISIT421 ORDER BY DESCRICAO ASC;", $db) or die(mysql_error());
									while($subrow = mysql_fetch_array($subresult)){
										echo "<option ";
										if (strcasecmp($vetor_CLASS421['CLASS421_TIPO'], $subrow['ID']) == 0){
											echo 'selected="selected"';
										} 
                            ...
                            <td width='30%'><textarea rows='4' class='form-control' name='CLASS421_DESCRICAO'><?php echo $vetor_CLASS421['CLASS421_DESCRICAO']; ?></textarea></td>
