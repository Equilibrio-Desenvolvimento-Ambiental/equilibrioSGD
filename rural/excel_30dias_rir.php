<?php
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$tabela .= '</thead>';
			$tabela .= '<tbody>';
			$sql = mysql_query("select * from VIEW_415421_ULTIMASVISITAS where DADOS_ATENDRIR = 1 and EVENTOS_DATA < DATE_SUB(NOW( ), INTERVAL 30 DAY) order by FAMILIA_BENEFICIARIO asc", $db); 
			...
			header("Content-Disposition: attachment; filename=excel_30dias_rir.xls");
