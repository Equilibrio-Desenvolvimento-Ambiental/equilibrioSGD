<?php
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$tabela .= '<th align="center"><b>B14 - Prinicipais Formas de Comercialização</b></th>';
			$tabela .= '</tr>';
			$tabela .= '</thead>';
			$tabela .= '<tbody>';
			$sql = mysql_query("SELECT TAB_FISH_FAMILIAS.FISH_FAM_ID, TAB_FISH_PERFILENT.FISH_PERFIL_QUEST, TAB_APOIO_BOOLEANO_STT_EMERG.DESCRICAO AS FISH_FAM_STT_EMERG, TAB_FISH_FAMILIAS.FISH_FAM_CHEFE_NOME, TAB_FISH_FAMILIAS.FISH_FAM_CHEFE_APELIDO, TAB_APOIO_STATUSATENDFISH.DESCRICAO AS FISH_PERFIL_STATUSATEND_DESC, TAB_FISH_CAMPANHAS.FISH_CAMP_DESCRICAO, TAB_FISH_ACOMP_ENTREVISTA.FISH_AE_DT_ENTR_ATUAL, TAB_APOIO_BOOLEANO_ANTES_PESCAVA.DESCRICAO AS FISH_AE_TXT_PESCAVA, TAB_APOIO_BOOLEANO_ANTES_VENDIA.DESCRICAO AS FISH_AE_TXT_VENDIA, TAB_APOIO_PESCA_COMERCIO.DESCRICAO AS FISH_AE_TXT_COMERCIALIZACAO FROM TAB_FISH_FAMILIAS LEFT OUTER JOIN TAB_APOIO_BOOLEANO AS TAB_APOIO_BOOLEANO_STT_EMERG ON TAB_FISH_FAMILIAS.FISH_FAM_STT_EMER = TAB_APOIO_BOOLEANO_STT_EMERG.ID LEFT OUTER JOIN TAB_FISH_PERFILENT ON TAB_FISH_PERFILENT.FISH_FAM_ID = TAB_FISH_FAMILIAS.FISH_FAM_ID LEFT OUTER JOIN TAB_FISH_ACOMP_ENTREVISTA ON TAB_FISH_ACOMP_ENTREVISTA.FISH_FAM_ID = TAB_FISH_FAMILIAS.FISH_FAM_ID LEFT OUTER JOIN TAB_FISH_CAMPANHAS ON TAB_FISH_ACOMP_ENTREVISTA.FISH_AE_CAMPANHA = TAB_FISH_CAMPANHAS.FISH_CAMP_ID LEFT OUTER JOIN TAB_FISH_ACOMP_AT_COMERCIO ON TAB_FISH_ACOMP_AT_COMERCIO.FISH_AAC_FAM = TAB_FISH_FAMILIAS.FISH_FAM_ID LEFT OUTER JOIN TAB_APOIO_PESCA_COMERCIO ON TAB_FISH_ACOMP_AT_COMERCIO.FISH_AAC_TIPO = TAB_APOIO_PESCA_COMERCIO.ID LEFT OUTER JOIN TAB_FISH_ACOMP_ANTESTRANS ON TAB_FISH_ACOMP_ANTESTRANS.FISH_FAM_ID = TAB_FISH_FAMILIAS.FISH_FAM_ID LEFT OUTER JOIN TAB_APOIO_BOOLEANO AS TAB_APOIO_BOOLEANO_ANTES_PESCAVA ON TAB_FISH_ACOMP_ANTESTRANS.FISH_AA_BOO_PESCAVA = TAB_APOIO_BOOLEANO_ANTES_PESCAVA.ID LEFT OUTER JOIN TAB_APOIO_BOOLEANO AS TAB_APOIO_BOOLEANO_ANTES_VENDIA ON TAB_FISH_ACOMP_ANTESTRANS.FISH_AA_BOO_PESCA_VENDIA = TAB_APOIO_BOOLEANO_ANTES_VENDIA.ID LEFT OUTER JOIN TAB_APOIO_STATUSATENDFISH ON TAB_FISH_PERFILENT.FISH_PERFIL_STATUSATEND = TAB_APOIO_STATUSATENDFISH.ID WHERE TAB_FISH_FAMILIAS.FISH_FAM_STT_EMER = 1 AND TAB_FISH_PERFILENT.FISH_PERFIL_QUEST IS NOT NULL ORDER BY TAB_FISH_PERFILENT.FISH_PERFIL_QUEST ASC, TAB_FISH_ACOMP_ENTREVISTA.FISH_AE_DT_ENTR_ATUAL DESC, FISH_AE_TXT_COMERCIALIZACAO ASC;", $db); 
			while ($vetor=mysql_fetch_array($sql)) {
				...
				$tabela .= '<td>'.$vetor['FISH_AE_TXT_PESCAVA'].'</td>';
				$tabela .= '<td>'.$vetor['FISH_AE_TXT_VENDIA'].'</td>';
				$tabela .= '<td>'.$vetor['FISH_AE_TXT_COMERCIALIZACAO'].'</td>';
				$tabela .= '</tr>';
			}
			$tabela .= '</tbody>';
			$tabela .= '</table>';
			...
			header("Content-Disposition: attachment; filename=excel_fish_acomp_quest_b12.xls");
