<?php
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$tabela .= '<th align="center"><b>Tipo da Despesa</b></th>';			
			$tabela .= '<th align="center"><b>Valor Mensal (R$)</b></th>';
			$tabela .= '</tr>';
			$tabela .= '</thead>';
			$tabela .= '<tbody>';
			$sql = mysql_query("SELECT TAB_FISH_FAMILIAS.FISH_FAM_ID, TAB_FISH_FAMILIAS.FISH_FAM_CHEFE_NOME, TAB_FISH_FAMILIAS.FISH_FAM_CONJ_NOME, TAB_FISH_COOP_ENTREVISTA.FISH_COOP_QUEST, TAB_APOIO_TECNICOS.DESCRICAO AS FISH_COOP_ENTREVISTA_DESC, TAB_FISH_COOP_ENTREVISTA.FISH_COOP_DTENTREVISTA, TAB_APOIO_DESPESAS.DESCRICAO AS FISH_COODES_DESPESA_DESC, TAB_FISH_COOP_DESPESAS.FISH_COODES_VALOR FROM TAB_FISH_FAMILIAS LEFT OUTER JOIN TAB_FISH_COOP_ENTREVISTA ON TAB_FISH_COOP_ENTREVISTA.FISH_FAM_ID = TAB_FISH_FAMILIAS.FISH_FAM_ID LEFT OUTER JOIN TAB_APOIO_TECNICOS ON TAB_FISH_COOP_ENTREVISTA.FISH_COOP_ENTREVISTA = TAB_APOIO_TECNICOS.ID LEFT OUTER JOIN TAB_FISH_COOP_DESPESAS ON TAB_FISH_COOP_DESPESAS.FISH_COOP_ID = TAB_FISH_COOP_ENTREVISTA.FISH_COOP_ID LEFT OUTER JOIN TAB_APOIO_DESPESAS ON TAB_FISH_COOP_DESPESAS.FISH_COODES_DESPESA = TAB_APOIO_DESPESAS.ID WHERE TAB_FISH_FAMILIAS.FISH_FAM_STT_COOP = 1 AND TAB_FISH_COOP_ENTREVISTA.FISH_COOP_ID IS NOT NULL ORDER BY TAB_FISH_COOP_ENTREVISTA.FISH_COOP_QUEST ASC, FISH_COODES_DESPESA_DESC ASC, TAB_FISH_COOP_DESPESAS.FISH_COODES_VALOR DESC;", $db) or die(mysql_error()); 
			while ($vetor=mysql_fetch_array($sql)) {
				...
				$tabela .= '<td>'.$vetor['FISH_COODES_DESPESA_DESC'].'</td>';
				$tabela .= '<td>'.number_format($vetor['FISH_COODES_VALOR'],2,',','.').'</td>';
				$tabela .= '</tr>';
			}
			$tabela .= '</tbody>';
			$tabela .= '</table>';
			...
			header("Content-Disposition: attachment; filename=excel_fish_despesas_completo.xls");
