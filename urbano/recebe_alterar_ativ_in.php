<?php 
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$sql = mysql_query("update TAB_444_ATIV_IN SET ATIVIN_DATA='$ATIV_DATA', ATIVIN_PARTICIPANTES='$ATIV_PARTICIPANTES', ATIVIN_RUC='$ATIV_RUC', ATIVIN_PERIODO='$ATIV_PERIODO', ATIVIN_EVENTO='$ATIV_EVENTO', ATIVIN_ATIVIDADE='$ATIV_ATIVIDADE', ATIVIN_DESCRICAO='$ATIV_DESCRICAO', ATIVIN_PERCEPCOES='$ATIV_PERCEPCAO' where ATIVIN_CODIGO = '$id'", $db) or die(mysql_error());
			
			$vet_TECNICOS = $_POST['ATIV_TECNICOS'];
			$cont_TECNICOS = 0;
			foreach((array)$vet_TECNICOS as $vet_TECNICOSkey){
				$tecnico = $_POST['ATIV_TECNICOS'][$cont_TECNICOS];
				$sql_TECNICOS = mysql_query("insert into TAB_444_ATIV_IN_TEC (ATIVIN_CODIGO, TECATIVIN_TECNICO) values ('$id', '$tecnico')", $db);
				$cont_TECNICOS++;
			}
			...
			foreach((array)$vet_ENTIDINST as $vet_ENTIDINSTkey){
				...
				$sql_ENTIDINST = mysql_query("insert into TAB_444_ATIV_IN_PART (ATIVIN_CODIGO, PARTATIVIN_PARTICIPANTE) values ('$id', '$entidade')", $db);
				$cont_ENTIDINST++;
			}
			...
			foreach((array)$vet_PERCEPCOES as $vet_PERCEPCOESkey){
				...
				$sql_PERCEPCOES = mysql_query("insert into TAB_444_ATIV_IN_PERCEP (ATIVIN_CODIGO, PERCEPATIVIN_PERCEP) values ('$id', '$percepcao')", $db);
				$cont_PERCEPCOES++;
			}
			...
			foreach((array)$vet_LEGENDAS as $vet_LEGENDASkey){
				...
				$sql_IMAGENS = mysql_query("insert into TAB_444_ATIV_IN_IMAGENS (ATIVIN_CODIGO, IMGATIVIN_NOME, IMGATIVIN_LEGENDA) values ('$id', '$nomefoto', '$legenda')", $db);
				$cont_LEGENDAS++;
			}
		/*
		...
		*/				
			echo"<script language=\"JavaScript\">
				location.href=\"listar_ativ_in.php\";
				</script>";
		}
	}
?>
