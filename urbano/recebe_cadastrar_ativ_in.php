<?php 
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$sql = mysql_query("insert into TAB_444_ATIV_IN (ATIVIN_DATA, ATIVIN_PARTICIPANTES, ATIVIN_RUC, ATIVIN_PERIODO, ATIVIN_EVENTO, ATIVIN_ATIVIDADE, ATIVIN_DESCRICAO, ATIVIN_PERCEPCOES) VALUES ('$ATIV_DATA', '$ATIV_PARTICIPANTES', '$ATIV_RUC', '$ATIV_PERIODO', '$ATIV_EVENTO', '$ATIV_ATIVIDADE', '$ATIV_DESCRICAO', '$ATIV_PERCEPCAO')", $db) or die(mysql_error());
			...
			foreach((array)$vet_TECNICOS as $vet_TECNICOSkey){
				...
				$sql_TECNICOS = mysql_query("insert into TAB_444_ATIV_IN_TEC (ATIVIN_CODIGO, TECATIVIN_TECNICO) values ('$id_gerado', '$tecnico')", $db);
				$cont_TECNICOS++;
			}
			...
			foreach((array)$vet_ENTIDINST as $vet_ENTIDINSTkey){
				...
				$sql_ENTIDINST = mysql_query("insert into TAB_444_ATIV_IN_PART (ATIVIN_CODIGO, PARTATIVIN_PARTICIPANTE) values ('$id_gerado', '$entidade')", $db);
				$cont_ENTIDINST++;
			}
			...
			foreach((array)$vet_PERCEPCOES as $vet_PERCEPCOESkey){
				...
				$sql_PERCEPCOES = mysql_query("insert into TAB_444_ATIV_IN_PERCEP (ATIVIN_CODIGO, PERCEPATIVIN_PERCEP) values ('$id_gerado', '$percepcao')", $db);
				$cont_PERCEPCOES++;
			}
			...
			foreach((array)$vet_LEGENDAS as $vet_LEGENDASkey){
				...
				$sql_IMAGENS = mysql_query("insert into TAB_444_ATIV_IN_IMAGENS (ATIVIN_CODIGO, IMGATIVIN_NOME, IMGATIVIN_LEGENDA) values ('$id_gerado', '$nomefoto', '$legenda')", $db);
				$cont_LEGENDAS++;
			}
		/*
