<?php 
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$sql = mysql_query("update TAB_444_ATIV_SC SET ATIVSC_DATA='$ATIV_DATA', ATIVSC_PARTICIPANTES='$ATIV_PARTICIPANTES', ATIVSC_RUC='$ATIV_RUC', ATIVSC_PERIODO='$ATIV_PERIODO', ATIVSC_EVENTO='$ATIV_EVENTO', ATIVSC_ATIVIDADE='$ATIV_ATIVIDADE', ATIVSC_DESCRICAO='$ATIV_DESCRICAO', ATIVSC_PERCEPCOES='$ATIV_PERCEPCAO' where ATIVSC_CODIGO = '$id'", $db) or die(mysql_error());
			
			$vet_TECNICOS = $_POST['ATIV_TECNICOS'];
			$cont_TECNICOS = 0;
			foreach((array)$vet_TECNICOS as $vet_TECNICOSkey){
				$tecnico = $_POST['ATIV_TECNICOS'][$cont_TECNICOS];
				$sql_TECNICOS = mysql_query("insert into TAB_444_ATIV_SC_TEC (ATIVSC_CODIGO, TECATIVSC_TECNICO) values ('$id', '$tecnico')", $db);
				$cont_TECNICOS++;
			}
			...
			foreach((array)$vet_ENTIDINST as $vet_ENTIDINSTkey){
				...
				$sql_ENTIDINST = mysql_query("insert into TAB_444_ATIV_SC_PART (ATIVSC_CODIGO, PARTATIVSC_PARTICIPANTE) values ('$id', '$entidade')", $db);
				$cont_ENTIDINST++;
			}
			...
			foreach((array)$vet_PERCEPCOES as $vet_PERCEPCOESkey){
				...
				$sql_PERCEPCOES = mysql_query("insert into TAB_444_ATIV_SC_PERCEP (ATIVSC_CODIGO, PERCEPATIVSC_PERCEP) values ('$id', '$percepcao')", $db);
				$cont_PERCEPCOES++;
			}
			...
			foreach((array)$vet_LEGENDAS as $vet_LEGENDASkey){
				...
				$sql_IMAGENS = mysql_query("insert into TAB_444_ATIV_SC_IMAGENS (ATIVSC_CODIGO, IMGATIVSC_NOME, IMGATIVSC_LEGENDA) values ('$id', '$nomefoto', '$legenda')", $db);
				$cont_LEGENDAS++;
			}
		/*
