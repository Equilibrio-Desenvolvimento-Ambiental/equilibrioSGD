<?php
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$id = $_GET['id'];
			$sql = mysql_query("select A.*, B.DESCRICAO as DESC_ATIVSC_RUC, C.DESCRICAO as DESC_ATIVSC_PERIODO,  D.DESCRICAO as DESC_ATIVSC_EVENTO, E.DESCRICAO as DESC_ATIVSC_ATIVIDADE from TAB_444_ATIV_SC A, TAB_APOIO_RUC B, TAB_APOIO_PERIODO C, TAB_APOIO_EVENUC_SC D, TAB_APOIO_ATIVNUC_SC E where A.ATIVSC_RUC = B.ID and A.ATIVSC_PERIODO = C.ID and A.ATIVSC_EVENTO = D.ID and A.ATIVSC_ATIVIDADE = E.ID and A.ATIVSC_CODIGO = '$id'", $db);
			$vetor = mysql_fetch_array($sql);
		
			$sql_ENTIDADES = mysql_query("select B.DESCRICAO from TAB_444_ATIV_SC_PART A, TAB_APOIO_ENTIDINST B where A.PARTATIVSC_PARTICIPANTE = B.ID and A.ATIVSC_CODIGO = '$id'", $db);
			$sql_TECNICOS = mysql_query("select B.DESCRICAO from TAB_444_ATIV_SC_TEC A, TAB_APOIO_TECNICOS B where A.TECATIVSC_TECNICO = B.ID and A.ATIVSC_CODIGO = '$id'", $db);
			$sql_PERCEPCOES = mysql_query("select B.DESCRICAO from TAB_444_ATIV_SC_PERCEP A, TAB_APOIO_PERCEPCOES B where A.PERCEPATIVSC_PERCEP = B.ID and A.ATIVSC_CODIGO = '$id'", $db);
			$sql_IMAGENS = mysql_query("select * from TAB_444_ATIV_SC_IMAGENS where ATIVSC_CODIGO = '$id'", $db);
?>
<!DOCTYPE html>
<html>
<head lang="en">
...
<body>
    ...
    <table>
        <tbody>
            ...
            <tr><td align="center"><h1>NÚCLEO SÓCIOCULTURAL</h1></td></tr>
	    </tbody>	
    </table><br/>
    <table>
    <tr align="left">
    <td width="12,5%"><strong>Data:</strong></td>
    <td width="12,5%"><?php echo date('d/m/Y', strtotime($vetor['ATIVSC_DATA'])); ?></td>
    <td width="12,5%"><strong>Participantes:</strong></td>
    <td width="12,5%"><?php echo $vetor['ATIVSC_PARTICIPANTES']; ?></td>
    <td width="12,5%"><strong>Bairro/RUC:</strong></td>
    <td width="12,5%"><?php echo $vetor['DESC_ATIVSC_RUC']; ?></td>
    <td width="12,5%"><strong>Período:</strong></td>
    <td width="12,5%"><?php echo $vetor['DESC_ATIVSC_PERIODO']; ?></td>
    </tr>
    </table><br/>
    <table>
    <tr align="left">
        <td width="15%"><strong>Tipo de Evento:</strong></td>
        <td><?php echo $vetor['DESC_ATIVSC_EVENTO']; ?></td>
    </tr>
    </table><br>
    <table>
    <tr align="left">
        <td width="15%"><strong>Tipo de Atividade:</strong></td>
        <td><?php echo $vetor['DESC_ATIVSC_ATIVIDADE']; ?></td>
    </tr>
    </table><br>
    <table>
    <tr align="left">
        <td width="15%"><strong>Descrição:</strong></td>
        <td><?php echo $vetor['ATIVSC_DESCRICAO']; ?></td>
    </tr>
    </table><br/>
    <table>
    <tr align="left">
        <td width="15%"><strong>Percepções Descritivas:</strong></td>
        <td><?php echo $vetor['ATIVSC_PERCEPCOES']; ?></td>
    </tr>
    </table><br/>
	<table>
	...
	<table>
		<tr><td colspan="3" bgcolor="#D9D9D9"><strong>4. REGISTRO FOTOGRÁFICO</strong></td></tr>
			<?php while ($vetor_IMAGENS = mysql_fetch_array($sql_IMAGENS)) { ?>
                ...
                <tr><td width="50"></td><td width="410"><img src="imagens/<?php echo $vetor_IMAGENS['IMGATIVSC_NOME'] ?>" width="400" alt=""/></td><td valign="top"><?php echo $vetor_IMAGENS['IMGATIVSC_LEGENDA'] ?></td></tr>
            <?php } ?>        
	</table><br/>
</body>
</html>
