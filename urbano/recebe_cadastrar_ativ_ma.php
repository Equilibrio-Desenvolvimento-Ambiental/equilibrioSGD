<?php 
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$sql = mysql_query("insert into TAB_444_ATIV_MA (ATIVMA_DATA, ATIVMA_PARTICIPANTES, ATIVMA_RUC, ATIVMA_PERIODO, ATIVMA_EVENTO, ATIVMA_ATIVIDADE, ATIVMA_DESCRICAO, ATIVMA_PERCEPCOES) VALUES ('$ATIV_DATA', '$ATIV_PARTICIPANTES', '$ATIV_RUC', '$ATIV_PERIODO', '$ATIV_EVENTO', '$ATIV_ATIVIDADE', '$ATIV_DESCRICAO', '$ATIV_PERCEPCAO')", $db) or die(mysql_error());
			...
			foreach((array)$vet_TECNICOS as $vet_TECNICOSkey){
				...
				$sql_TECNICOS = mysql_query("insert into TAB_444_ATIV_MA_TEC (ATIVMA_CODIGO, TECATIVMA_TECNICO) values ('$id_gerado', '$tecnico')", $db);
				$cont_TECNICOS++;
			}
			...
			foreach((array)$vet_ENTIDINST as $vet_ENTIDINSTkey){
				...
				$sql_ENTIDINST = mysql_query("insert into TAB_444_ATIV_MA_PART (ATIVMA_CODIGO, PARTATIVMA_PARTICIPANTE) values ('$id_gerado', '$entidade')", $db);
				$cont_ENTIDINST++;
			}
			...
			foreach((array)$vet_PERCEPCOES as $vet_PERCEPCOESkey){
				...
				$sql_PERCEPCOES = mysql_query("insert into TAB_444_ATIV_MA_PERCEP (ATIVMA_CODIGO, PERCEPATIVMA_PERCEP) values ('$id_gerado', '$percepcao')", $db);
				$cont_PERCEPCOES++;
			}
			...
			foreach((array)$vet_LEGENDAS as $vet_LEGENDASkey){
				...
				$sql_IMAGENS = mysql_query("insert into TAB_444_ATIV_MA_IMAGENS (ATIVMA_CODIGO, IMGATIVMA_NOME, IMGATIVMA_LEGENDA) values ('$id_gerado', '$nomefoto', '$legenda')", $db);
				$cont_LEGENDAS++;
			}
		/*
