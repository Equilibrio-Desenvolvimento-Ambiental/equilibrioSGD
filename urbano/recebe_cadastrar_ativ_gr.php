<?php 
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$sql = mysql_query("insert into TAB_444_ATIV_GR (ATIVGR_DATA, ATIVGR_PARTICIPANTES, ATIVGR_RUC, ATIVGR_PERIODO, ATIVGR_EVENTO, ATIVGR_ATIVIDADE, ATIVGR_DESCRICAO, ATIVGR_PERCEPCOES) VALUES ('$ATIV_DATA', '$ATIV_PARTICIPANTES', '$ATIV_RUC', '$ATIV_PERIODO', '$ATIV_EVENTO', '$ATIV_ATIVIDADE', '$ATIV_DESCRICAO', '$ATIV_PERCEPCAO')", $db) or die(mysql_error());
			...
			foreach((array)$vet_TECNICOS as $vet_TECNICOSkey){
				...
				$sql_TECNICOS = mysql_query("insert into TAB_444_ATIV_GR_TEC (ATIVGR_CODIGO, TECATIVGR_TECNICO) values ('$id_gerado', '$tecnico')", $db);
				$cont_TECNICOS++;
			}
			...
			foreach((array)$vet_ENTIDINST as $vet_ENTIDINSTkey){
				...
				$sql_ENTIDINST = mysql_query("insert into TAB_444_ATIV_GR_PART (ATIVGR_CODIGO, PARTATIVGR_PARTICIPANTE) values ('$id_gerado', '$entidade')", $db);
				$cont_ENTIDINST++;
			}
			...
			foreach((array)$vet_PERCEPCOES as $vet_PERCEPCOESkey){
				...
				$sql_PERCEPCOES = mysql_query("insert into TAB_444_ATIV_GR_PERCEP (ATIVGR_CODIGO, PERCEPATIVGR_PERCEP) values ('$id_gerado', '$percepcao')", $db);
				$cont_PERCEPCOES++;
			}
			...
			foreach((array)$vet_LEGENDAS as $vet_LEGENDASkey){
				...
				$sql_IMAGENS = mysql_query("insert into TAB_444_ATIV_GR_IMAGENS (ATIVGR_CODIGO, IMGATIVGR_NOME, IMGATIVGR_LEGENDA) values ('$id_gerado', '$nomefoto', '$legenda')", $db);
				$cont_LEGENDAS++;
			}
		/*
