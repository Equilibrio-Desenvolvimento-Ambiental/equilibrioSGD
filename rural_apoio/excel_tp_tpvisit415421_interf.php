<?php
	...
	if(!isset($_SESSION['user']) || !isset($_SESSION['login']) || !isset($_SESSION['senha']) || !isset($_SESSION['nome']) || !isset($_SESSION['nivel'])) {
		...
		if ($num_busca == 0) {
			...
			$tabela .= '</thead>';
			$tabela .= '<tbody>';
			$sql = mysql_query("SELECT * FROM TAB_APOIO_TPVISIT415421_INTERF ORDER BY ID ASC;", $db) or die(mysql_error());
			while ($vetor=mysql_fetch_array($sql)) {
				$tabela .= '<tr>';
				$tabela .= '<td>'.$vetor['ID'].'</td>';
				$tabela .= '<td>'.$vetor['DESCRICAO'].'</td>';
				if (strcasecmp($vetor['ATIVO'],'1') == 0) {
					$tabela .= '<td>SIM</td>';
				} else {
					$tabela .= '<td>NÃO</td>';
				};
			...
			header("Content-Disposition: attachment; filename=excel_tp_tpvisit415421_interf.xls");
